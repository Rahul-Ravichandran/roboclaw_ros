...
class Node:
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            index = count()
            def animate(i):
                global p1,p2,get_cmd_vel,U, xvals, yvals1, yvals2, wheel_pwm_err_prev, wheel_pwm_int_err, kp, kd,ki
                max_possible_speed=28.19
                ...
                try:
                    if U[0] is 0 and U[1] is 0 and U[2] is 0 and U[3] is 0:
                        ...
                        # Kd_err = 0.01
                        # Ki_err = 0.5
                        wheel_pwm_acc = U + Kp_err*wheel_pwm_err+Kd_err*wheel_pwm_deriv_err + Ki_err*wheel_pwm_int_err
                        ...
                        ####limits
                        if wheel_pwm_acc[0] > max_possible_speed:
                            wheel_pwm_acc[0] = max_possible_speed
                        if wheel_pwm_acc[0] < -max_possible_speed:
                            wheel_pwm_acc[0] = -max_possible_speed
                        if wheel_pwm_acc[1] > max_possible_speed:
                            wheel_pwm_acc[1] = max_possible_speed
                        if wheel_pwm_acc[1] < -max_possible_speed:
                            wheel_pwm_acc[1] = -max_possible_speed
                        if wheel_pwm_acc[2] > max_possible_speed:
                            wheel_pwm_acc[2] = max_possible_speed
                        if wheel_pwm_acc[2] < -max_possible_speed:
                            wheel_pwm_acc[2] = -max_possible_speed
                        if wheel_pwm_acc[3] > max_possible_speed:
                            wheel_pwm_acc[3] = max_possible_speed
                        if wheel_pwm_acc[3] < -max_possible_speed:
                            wheel_pwm_acc[3] = -max_possible_speed
